    ...
    <section
        ...
        <div class="px-16 py-10 bg-white rounded-2xl flex flex-col gap-4">
            <div class="flex flex-col justify-center items-center text-center">
                <h1 class="font-extrabold font-serif text-2xl text-black">Selamat Datang</h1>
                <p class="text-base font-medium text-gray-800">Kami Berharap Kamu masuk dan Menikmati.</p>
            </div>
            <form method="POST" action="{{ route('login') }}" class="space-y-5">
                ...
                @endif
                @if (session('error'))
                    <div class="text-xl font-semibold text-center text-red-600">
                        {{ session('error') }}
                    </div>
                @endif
                
                <p class="text-base text-center font-semibold text-gray-700 pt-4">Belum Punya Akun? <a href="{{ route('register') }}" class="text-blue-800">Daftar</a></p>
            </form>
        </div>
